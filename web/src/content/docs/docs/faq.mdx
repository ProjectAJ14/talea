...
`~/.talea/talea.repos.json` is the catalogue, `~/.talea/state.json` remembers the gist id and the update check, and `<workspace>/.talea.json` is what this machine keeps. Delete the `~/.talea` directory and talea has forgotten everything except the checkouts, which are still where they were.
